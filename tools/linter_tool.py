...
class LinterTool(BaseTool):
    ...
    async def run(self, path: str, fix: bool = False, **kwargs) -> ToolResult:
        ...
        success = lint_result.success or fix  # If fix mode, success even if changes made

        return ToolResult(
            success=success,
            output=combined_output,
            error="\n".join(filter(None, [lint_result.error, format_result.error])) or None,
            metadata={
    ...
    def _count_issues(self, ruff_output: str) -> int:
        ...
        return sum(1 for l in lines if l.strip() and not l.startswith("Found") and ": " in l)
...
class SecurityScanTool(BaseTool):
    ...
    async def run(self, path: str, severity: str = "medium", **kwargs) -> ToolResult:
        ...
        """
        severity_level = {"low": "l", "medium": "m", "high": "h"}.get(severity, "m")
        ...
        return ToolResult(
            ...
            error=f"{high_count} high-severity issues found" if high_count > 0 else None,
            metadata={
