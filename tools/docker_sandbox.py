...
class DockerSandboxTool(BaseTool):
    ...
    async def run(self, action: str, cmd: str, image: str = "python:3.11-slim", allow_internet: bool = False, **kwargs) -> ToolResult:
        if action == "execute":
    ...
    async def _execute_sandboxed(self, cmd: str, image: str, allow_internet: bool, **kwargs) -> ToolResult:
        """
        Spins up an ephemeral container mapped to the project workspace.
        """
        container_name = f"ai-org-sandbox-{uuid.uuid4().hex[:8]}"
        logger.info("Spawning execution sandbox", container=container_name[:12], image=image)

        # Build the docker run command to ensure isolation
        docker_cmd = [
            "docker", "run", "--rm",
            "--name", container_name,
            "-v", f"{os.path.abspath(self.working_dir)}:/workspace",
            "-w", "/workspace",
        ]
        ...
        if result.success:
            ...
            logger.warning("Sandbox execution failed", container=container_name[:12], error=result.error)
