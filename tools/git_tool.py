...
class GitTool(BaseTool):
    ...
    async def _init(self, path: str | None = None) -> ToolResult:
        ...
        if result.success:
            ...
            await self._run_subprocess(
                ["git", "config", "user.email", "swarm@sarang.local"], cwd=target
            )
            await self._run_subprocess(
    ...
    async def _push(self, remote: str = "origin", branch: str = "main") -> ToolResult:
        ...
        if token:
            ...
            logger.info(
                "Pushing to GitHub using token authentication",
                remote=remote,
                branch=branch,
            )
            # We check if 'origin' is set to a HTTPS URL and inject the token
            status_res = await self._run_subprocess(
                ["git", "remote", "get-url", remote], cwd=self.repo_path
            )
            if status_res.success:
                ...
                if url.startswith("https://github.com/"):
                    ...
                    await self._run_subprocess(
                        ["git", "remote", "set-url", remote, url], cwd=self.repo_path
                    )
                    return res
