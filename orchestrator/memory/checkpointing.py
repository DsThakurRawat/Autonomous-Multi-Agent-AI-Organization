...
class CheckpointManager:
    ...
    async def save_checkpoint(
        ...
        if not diff_res.output.strip():
            logger.debug("No operational changes detected for checkpoint.", task_name=task_name)
            return
        ...
        if res.success:
            logger.info("Shadow checkpoint committed successfully.", task_name=task_name, agent=agent_role)
        else:
    ...
    async def rewind(self, commit_hash: str) -> Optional[Dict[str, Any]]:
        ...
        if not res.success:
            logger.error("Failed to rewind workspace", hash=commit_hash, error=res.error)
            return None
